...
def load_from_pickle(filename):
    with open(filename, 'rb') as f:
        pickle_dataset = pickle.load(f, encoding='latin1')
    
    dataset = {k: getattr(pickle_dataset, k) for k in vars(pickle_dataset).keys()}
    dataset['T'] = [x.astype('int') for x in dataset['T']]
    return dataset
...
def get_otu_ids(phylo_tree, var_names):
    ...
    otu_idx = [var_names.index(n) for n in otu_names]
...
def get_dist_matrix(phylo_tree, var_names, otu_idx):
    ...
    for i, src in enumerate(phylo_tree.get_leaves()):
        # i = var_names.index(src.name)
        # i = otu_idx.index(i)
        for j, dst in enumerate(phylo_tree.get_leaves()):
...
# else zero
def get_data_matrix(num_otus, num_time, samples, times, otu_idx, labels, use_abun=False):
...
########################### 16s and Metaphlan data loading ############################
